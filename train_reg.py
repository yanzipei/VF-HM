...
from dataset import VFDataset, eye_types
from model import model_names, Regressor
from utils import save_checkpoint, set_seed, save_yaml_file, print_info, AverageMeter, ProgressMeter, \
    adjust_learning_rate

parser = argparse.ArgumentParser(description='Regression baseline for VF estimation using fundus')
...
def train(train_loader, model, optimizer, epoch, print_freq):
    ...
    for i, (fundus, vf) in enumerate(train_loader):
...
def main():
    ...
    train_dataset = VFDataset(csv_file=args.train_csv_file,
    ...
    test_dataset = VFDataset(csv_file=args.test_csv_file,
    ...
    model = Regressor(arch=args.arch,
    ...
    for epoch in range(args.epochs):
        ...
        train(train_loader, model, optimizer, epoch, args.print_freq)
